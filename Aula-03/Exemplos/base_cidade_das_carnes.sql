...
select * from tb_produtos where not categoria_id = 1; -- mostra todos o produtos menos as que estão inseridas na categoria id = 1

select * from tb_produtos where nome like "ha%"; -- desde que seja em formato de texto, mostra tudo que começa com "HA"

select * from tb_produtos where nome like "%ha"; -- desde que seja em formato de texto, mostra tudo que termina com "HA"

select * from tb_produtos where nome like "%ha%"; -- desde que seja em formato de texto, mostra tudo que contenha a sílaba "HA"

select * from tb_produtos where nome  like "%ha%" order by nome; -- desde que seja em formato de texto, mostra tudo que contenha a sílaba "HA" de forma ordenada

show create table tb_produtos; -- PESQUISAR O QUE É

select * from tb_produtos where preco in(20,30,40); -- Forma mais simples de mostrar dados com valores nessa faixa de preço // dá também para trabalhar com data
...
select  categoria_id, avg(preco) from tb_produtos group by categoria_id; -- calcula a média por categoria id
...
on tb_categoria.id = tb_produtos.categoria_id; -- pirmiero o que quero motrar indicando de onde são esses atributos e quais tabelas conjuntaas e com quem vai ser feita essa relação, ou seja quem é a chave estrangeira de um lado e primária do outro
